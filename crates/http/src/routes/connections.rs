...
pub(crate) async fn get_affinity(Path(domain): Path<String>) -> Json<Affinity> {
    Json(ethui_connections::commands::connections_affinity_for(domain).await)
}
...
pub(crate) async fn set_affinity(
    ...
    ethui_connections::commands::connections_set_affinity(&domain, payload.affinity).await?;

...
use ethui_rpc_macros::camel_to_snake;
...
impl Handler {
    ...
    fn add_handlers(&mut self) {
        macro_rules! self_handler {
            ($name:tt) => {
                self_handler!($name, $name);
            };
            ($name:ident, $fn:ident) => {
                self.io
                    .add_method_with_meta(stringify!($name), |params: Params, ctx: Ctx| async move {
                        info!(method = stringify!($name), params = serde_json::to_string(&params).unwrap());
                        let ret = Self::$fn(params, ctx).await;
                        info!(result = ?ret);
                        ret
                    });
            };
        ...
        macro_rules! provider_handler {
            ($name:ident) => {
                self.io.add_method_with_meta(
                    stringify!($name),
                    |params: Params, ctx: Ctx| async move {
                        let provider = ctx.network().await.get_provider();

                        let res: jsonrpc_core::Result<serde_json::Value> = provider
                            .raw_request::<_, serde_json::Value>(stringify!($name).into(), params)
                            .await
        ...
        // handle internally
        self_handler!(eth_accounts);
        self_handler!(eth_requestAccounts, eth_accounts);
        self_handler!(eth_chainId);
    ...
    async fn eth_accounts(_: Params, _: Ctx) -> jsonrpc_core::Result<serde_json::Value> {
    ...
    async fn eth_chain_id(_: Params, ctx: Ctx) -> jsonrpc_core::Result<serde_json::Value> {

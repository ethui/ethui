...
import { Wallet2, Wallets, walletSchema2, walletsSchema } from "../types";

const emptyWallet: Wallet2 = {
  name: "",
  dev: false,
  mnemonic: "",
  derivationPath: "",
...
export function SettingsWallets() {
  ...
  const { data: wallets, mutate } = useInvoke<Wallet2[]>("wallets_get_all");
  ...
  const onSubmit = useCallback(
    async (data: { wallets?: Wallet2[] }) => {
      reset(data);
      await invoke("wallets_set", { wallets: data.wallets });
      mutate();
    },
    [reset, mutate]
  );
          ...
          return (
            <Stack spacing={2} alignItems="flex-start" key={field.id}>
              ...
              <TextField
                ...
                {...register(`wallets.${index}.idx`, { valueAsNumber: true })}
              />
              <Button
                color="warning"
                size="small"

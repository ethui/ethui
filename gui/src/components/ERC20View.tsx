...
import { Stack, IconButton, Card, CardHeader, Menu, MenuItem } from "@mui/material";
...
export function ERC20View({ chainId, contract, symbol, balance, decimals }: Props) {
  ...
  const onMenuOpen = (event: React.MouseEvent<HTMLElement>) => setMenuAnchor(event.currentTarget);

  const blacklist = ()=>{

    invoke("db_set_erc20_blacklist", {chainId:network.chain_id, address: contract, blacklisted: true })
    setMenuAnchor(null);
  }
  ...
  return <Card elevation={0}>
    ...
    <Menu open={Boolean(menuAnchor)} id={`erc20-${contract}-menu`} anchorEl={menuAnchor} onClose={() => setMenuAnchor(null)} >
      {contract && network?.explorer_url && <MenuItem component="a" target="_blank" href={`${network.explorer_url}${contract}`}>Open on explorer</MenuItem>}
